...
7.
rebase는 한 브랜치의 commit을 다른 브랜치의 가장 최신 commit 위로 이동시켜, 브랜치의 변경 이력을 재작성하여 더 깔끔하고 직선적인 이력을 생성한다. 따라서 여러 개의 커밋을 한 브랜치에서 하나로 합쳐서 깔끔한 이력을 만들고 싶을 때 유용하다.

8.
git stash 는 현재 작업 중인 변경 사항을 임시로 저장하는 명령어이다. 현재 작업을 임시 저장하고 working directory를 깨끗한 상태로 되돌리므로, 현재 작업을 일시 중단하고 다른 작업으로 전환하고 싶을 때 사용한다. git stash list/apply/drop/pop 등의 명령어로 활용할 수 있다.