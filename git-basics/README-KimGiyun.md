...
### origin
- 본인이 사용하는 remote repository의 일반적인 별칭. 
...
### Pull Requset
- 자신의 코드 변경사항을 협업 중인 팀원과 공유하고, 해당 변경 사항을 main에 통합(merge)하도록 요청하는 과정.
- Issue의 해결을 위해 내 local에서 작업했고, 해당 변경 사항을 메인에 반영하고 싶다(Pull)고 요청(Request)하는 행위.
    - 변경 내역을 상호 공유하고, 리뷰, 토론, 수정하는 단계를 거칠 수도 있음.
- Pull Request 과정
    1. Repository를 clone해 local에 저장하고, branch를 생성.
    2. 새로 생성한 브랜치에서 파일 수정 및 기능 추가. 
    3. 소스 코드를 포함한 branch를 remote repository로 push.
    4. Github에 접속하여 Fork한 저장소에 Compare & pull request 버튼이 활성화되어 있는지 확인. 
    5. 해당 버튼을 눌러 Pull Request를 생성.
    6. Pull Request 제목과 설명을 작성 및, 팀원들을 리뷰어로 지정 
    7. 변경 사항을 검토하고 상호 논의 및 피드백.
    8. 프로젝트 매니저가 변경 사항을 병합할 수 있도록 Pull Request를 제출.

### Merge
- 서로 다른 branch를 병합하여 작업한 내용을 합치는 것.
    - branch로 분기하여 독자적으로 개발한 뒤, 다시 메인으로 병합할 때 사용.
- Merge 타입
    1. Fast-Forward
        - branch의 HEAD가 Merge 과정 없이 최신 커밋으로 곧장 이동하는 방식.
        - branch 간 병합 진행 시 별도 commit이 생기지 않음.
        - 분기한 branch의 commit history가 기존 branch의 commit history를 포함하고 있을 때 Fast Forward 관계를 가진다고 표현.
        - 새 branch를 생성 한 후, base에서 더 이상 commit하지 않고, 새 branch에서만 commit하는 경우 사용.
        - git merge <merge 할 branch명>
    ![fast-forward / 출처:eunbeann.log](https://velog.velcdn.com/images/eunbeann/post/3b5e736a-644a-4f0b-9c07-0660f3586e4b/image.png)
    2. 3-way Merge
        - 현재 branch의 commit, 병합할 branch의 commit, 두 branch의 공통 조상이 되는 Base commit을 비교해 merge를 수행.
            - Fast-Forward 병합이 불가능한 상황에 수행하는 병합 방식.
            -  Base Commit 기준, 현재 branch와 병합할 branch의 코드를 비교하여 자동으로 병합하고, 충돌하는 부분은 개발자가 직접 병합.
            - Base commit 없이 두 branch를 병합하려면, 어떤 branch의 소스 코드가 수정된 코드인지 파악하기 어렵기에, 공통 조상인 Base와 소스 코드를 비교해 수정된 부분을 찾는 3-way 방식을 사용.
            ![3-way merge / 출처 : eunbeann.log](https://velog.velcdn.com/images/eunbeann/post/9d164de2-1220-4de5-b823-793ceca6bdcf/image.png)
            ![2-way / 출처 : tlatmsrud.tistory.com/155](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FeqrBYn%2FbtsEdTOgFrb%2F0XpB0WBHSiB6aH6BcxDy3k%2Fimg.png)
            ![3-way / 출처: tlatmsrud.tistory.com/155](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2Fz7CWP%2FbtsEa5hQd1W%2FKi4KgsWsQlk6TcdWJX9FbK%2Fimg.png)

## rebase
![rebase](https://user-images.githubusercontent.com/51331195/160234052-7fe70f85-5906-4474-b809-782adae92b3c.png)  
- branch의 Base Commit(= Base)를 재설정하는 명령어
    - A branch에서 B branch에 대한 Rebase를 할 경우 A branch의 Base Commit이 B branch의 Head Commit으로 변경됨.
- 장점
    - Fast Forward Merge를 통해 commit history를 직렬 형태로 깔끔하게 유지할 수 있음.
    - 공유 branch의 최신 변경사항을 즉각 반영할 수 있음.
- 협업 중인 branch를 rebase 했다가, 해당 branch로 작업 중인 개발자가 피해볼 수 있으므로 문제 될 경우가 없을 때만 사용.

## stash
![stash](https://d8it4huxumps stack.cloudfront.net/bites/wp-content/banners/2023/4/642a663eaff96_git_stash.png)  
- 작업 중이던 코드의 변경 사항을 stack에 잠시 저장할 수 있도록 하는 명령어.
    - 미완료된 작업을 commit 없이 보관해두었다가, stack에서 꺼내와 마무리 할 수 있음.
- Working Directory에서 수정한 파일만을 stack에 저장.
    - Modified이면서 Tracked 상태의 파일, Staged 상태의 파일을 저장.
```bash
    # 새 stash를 stack에 만들어 작업 내용을 임시 저장 
    git stash
