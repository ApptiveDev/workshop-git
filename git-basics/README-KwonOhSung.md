...
이미 git init으로 로컬 저장소를 만들고 나서 원격 저장소를 연결하려면 git remote add 명령을 사용하여 origin을 설정할 수 있습니다.
...
원격 저장소의 변경 사항을 로컬 저장소로 가져오지만, 현재 브랜치에는 자동으로 병합하지 않습니다. 원격 브랜치의 최신 상태를 로컬 저장소의 원격 추적 브랜치(origin/branch-name)로 업데이트합니다.

git pull

원격 저장소에서 변경 사항을 가져오고(git fetch), 현재 브랜치에 자동으로 병합합니다. 즉, git fetch와 git merge를 한 번에 수행합니다.


git fetch의 활용 상황

변경 사항 검토 및 비교: 원격 저장소의 최신 변경 사항을 로컬에 가져와서, 이를 로컬 브랜치에 병합하기 전에 변경 사항을 검토할 수 있습니다.

충돌 방지 및 안전한 병합:

원격 저장소의 변경 사항을 가져온 후, 충돌을 수동으로 해결하거나 코드 리뷰를 진행한 후에 병합을 수행합니다.

병합 충돌을 미리 방지하고, 예상하지 못한 변경 사항을 사전에 파악할 수 있습니다.

정기적인 원격 데이터 동기화: 원격 저장소의 상태를 정기적으로 확인하고, 최신 브랜치나 태그를 가져오는 데 사용됩니다.

로컬 변경 사항 유지: 로컬 작업 중 원격 저장소의 상태를 알고 싶지만, 현재 작업 중인 브랜치에 변경 사항을 적용하고 싶지 않을 때 사용합니다.


- `reset --hard`와 `push --force`의 적절한 사용법

git reset --hard는 현재 브랜치를 지정한 커밋으로 이동시키고, 작업 디렉토리와 스테이징 영역을 해당 커밋의 상태로 되돌립니다. 즉, 커밋, 스테이징 영역, 작업 디렉토리의 모든 변경 사항이 제거됩니다.

적절한 사용 상황

로컬 작업 정리: 로컬 브랜치에서 실험적 변경을 많이 했지만, 이제는 초기 상태로 되돌리고 싶을 때.

잘못된 커밋 제거: 최근의 잘못된 커밋을 제거하고 싶을 때, 변경 사항을 모두 삭제하고 이전 상태로 되돌리려는 경우.


git push --force는 로컬 브랜치의 변경 사항을 원격 브랜치에 강제로 푸시합니다. 원격 브랜치의 변경 사항을 덮어쓰므로, 원격 저장소의 히스토리가 변경됩니다.

적절한 사용 상황

로컬 브랜치 재작성: 로컬 브랜치에서 git reset --hard를 사용하여 커밋을 재작성한 후, 원격 브랜치에도 동일하게 적용할 때.

커밋 히스토리 정리: 예를 들어, git rebase로 히스토리를 정리한 후, 원격 저장소에 변경된 히스토리를 반영할 때.
